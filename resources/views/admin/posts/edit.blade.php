...
<div class="max-w-2xl mx-auto bg-white p-8 rounded-2xl shadow-sm border">
    ...
    <form class="space-y-5" action="{{ route('admin.update', $post)}}"  method="post">
        @csrf
        @method('PUT')
        <div>
            <label class="block text-sm font-medium mb-1">Titre</label>
            <input type="text" name="title" value="{{$post->title}}" class="w-full border p-3 rounded-lg focus:ring-2 focus:ring-orange-500 outline-none">
        </div>
        ...
        <div>
            ...
            <textarea rows="5" name="contenu" class="w-full border p-3 rounded-lg focus:ring-2 focus:ring-orange-500 outline-none">{{ $post->contenu }}</textarea>
        </div>
